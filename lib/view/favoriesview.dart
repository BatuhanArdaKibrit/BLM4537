...
class FavoriesView extends StatefulWidget {

  const FavoriesView({super.key});

  @override
  State<FavoriesView> createState() => _FavoriesViewState();
}

class _FavoriesViewState extends State<FavoriesView> {
  List<RecipeModel>? recipeModel;
  List<RecipeModel>? filterModel;
  TextEditingController searchController = TextEditingController(text: "");
  @override
  void initState() {
    ApiCallViewModel().getLikedFood(Globals.email).then((value) {
      if (value != null) {
        setState(() {
          recipeModel = value;
          filterModel = value;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        Center(child: Text("Favorilerinizde tarif bulunmamaktadır.",style: TextStyle(color: Colors.white,fontSize: 18.sp))) :
        SafeArea(
          child: Column(
            children: [
              Container(
